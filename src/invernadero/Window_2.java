...
public class Window_2 extends JFrame {
	...
	public Window_2() {
		...
		display2.setBounds(10, 98, 299, 263);
		contentPane.add(display2);
		
		text2 = new JTextArea();
		text2.setBounds(10, 372, 203, 22);
		...
		send2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(s.equals("/planta")) {
					...
					display2.append("\nLas plantas del invernadero de " + Window_1.Sucursal1 + "\nestán siendo mostradas en la pantalla.\nPara volver a las de su sucursal\npresione en el botón Refrescar." + "\n\n");
					text2.setText("");
					return;
				}
				display2.append(Sucursal2 + ": " + s + "\n");
				Window_1.sendText();
				text2.setText("");
				
			}
		});
		send2.setBounds(220, 372, 89, 22);
		contentPane.add(send2);
		
		label2 = new JLabel("Chat con el Invernadero de " + Window_1.Sucursal1);
		label2.setFont(new Font("Tahoma", Font.BOLD, 11));
		label2.setBounds(10, 64, 203, 30);
	...
	btnNewButton.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			Connection con = null;
			String urlDatabase = "jdbc:postgresql://localhost:5432/Invernadero2";
			try {
				Class.forName("org.postgresql.Driver");
				con = DriverManager.getConnection(urlDatabase, user, password);
				Statement st = con.createStatement();
				String sql = "INSERT INTO Invernadero2(serial, nombre, grupo, orden, familia, genero, cantidad)"
	...
	btnModificar.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				con = DriverManager.getConnection(urlDatabase, user, password);
				Statement st = con.createStatement();
				String sql = "UPDATE Invernadero2 SET"
	...
	btnEliminar.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				con = DriverManager.getConnection(urlDatabase, user, password);
				Statement st = con.createStatement();
				String sql = "DELETE FROM Invernadero2 WHERE serial='" + textSerial.getText() + "'";
				ResultSet result = st.executeQuery(sql);
				con.close();
				st.close();
			} catch (Exception e1) {
	...
	public static void sendText() {
		...
		display2.append(Window_1.Sucursal1 + ": " + s + "\n");
}
